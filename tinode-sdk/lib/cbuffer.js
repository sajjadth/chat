...
class CBuffer {
    ...
    put(...values) {
        ...
        for (const key in insert) {
            if (Object.prototype.hasOwnProperty.call(insert, key)) {
                utilities_1.Utilities.insertSorted(insert[key], this.buffer, this.unique, this.compare);
            }
        }
    }
    /**
     ...
     */
    find(elem, nearest) {
        const { idx } = utilities_1.Utilities.findNearest(elem, this.buffer, !nearest);
        return idx;
    }
}
