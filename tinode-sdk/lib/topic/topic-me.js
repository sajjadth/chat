...
exports.TopicMe = void 0;
const constants_1 = require("../constants");
const access_mode_1 = require("../access-mode");
const utilities_1 = require("../utilities");
const topic_1 = require("./topic");
const rxjs_1 = require("rxjs");
class TopicMe extends topic_1.Topic {
    constructor(tinode) {
        super(constants_1.TopicNames.TOPIC_ME, tinode);
        // List of contacts
        this.contacts = {};
        // Events
        this.onContactUpdate = new rxjs_1.Subject();
        /**
         * Called by Tinode when meta.sub is received.
         */
        this.processMetaCreds = (creds, upd) => {
            if (creds.length === 1 && creds[0] === constants_1.DEL_CHAR) {
                creds = [];
            }
            if (upd) {
                creds.forEach((cr) => {
    ...
    processMetaSub(subs) {
        for (const key in subs) {
            if (Object.prototype.hasOwnProperty.call(subs, key)) {
                ...
                if (topicName === constants_1.TopicNames.TOPIC_FND || topicName === constants_1.TopicNames.TOPIC_ME) {
                    continue;
                }
                ...
                if (sub.deleted) {
                    ...
                    cont = utilities_1.Utilities.mergeToCache(this.contacts, topicName, sub);
                    if (utilities_1.Utilities.isP2PTopicName(topicName)) {
    ...
    routePres(pres) {
        ...
        if (pres.what === 'upd' && pres.src === constants_1.TopicNames.TOPIC_ME) {
        ...
        if (cont) {
            switch (pres.what) {
                ...
                case 'acs': // access mode changed
                    if (cont.acs) {
                        ...
                        cont.acs = new access_mode_1.AccessMode().updateAll(pres.dacs);
                    }
                    cont.touched = new Date();
                    break;
            ...
            if (pres.what === 'acs') {
                ...
                const acs = new access_mode_1.AccessMode(pres.dacs);
                if (!acs || acs.mode === constants_1.AccessModeFlags.INVALID) {
                    this.tinode.logger('ERROR: Invalid access mode update', pres.src, pres.dacs);
                    return;
                }
                else if (acs.mode === constants_1.AccessModeFlags.NONE) {
     ...
     */
    getMsgReadRecv(contactName, what) {
        const cont = this.contacts[contactName];
        if (cont) {
            switch (what) {
                case 'recv':
    ...
    getAccessMode() {
        if (String(name)) {
            const cont = this.contacts[Number(name)];
